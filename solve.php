...
function is_possible_number($cell,$number,$sudoku){
	...
	return is_possible_row($number,$row,$sudoku) and is_possible_col($number,$col,$sudoku) and 
is_possible_block($number,$block,$sudoku);
}	
...
for ($x=0 ; $x<9 ; $x++)                       // Rows
        {
        for ($y=0 ; $y<9 ; $y++)               // Values in rows (columns)
                {
                ...
                $name = $x.",".$y;
                        ...
                        if(empty($_POST[$name]))
                                {
                                $value = "0";   // If the field is empty, assign value x
                                }
...
//              echo $name . " - POST value: ". $value . "<br>";
                }


        }

$sudoku = str_split($trim);
 
solve($sudoku);

	echo "<p>" . $trim;
	echo '$e len: ' . strlen($e). "<br>";
	echo '$trim len: '. strlen($trim). "<br>";
	echo '$sudoku len: '. strlen($sudoku). "<br>";
